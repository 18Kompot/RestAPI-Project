...
async function localMongoConnect() {
  try {
    ...
    console.log(
      chalk.bgGreenBright(
        `Connected to Mongodb Local DataBase Server, DataBase: ${dataBase}`
      )
    );
  } catch (error) {
...
async function cloadMongoConnect() {
  try {
    await mongoose.connect(
      `mongodb+srv://${process.env.CLOAD_MONGODB_USER}:${process.env.CLOAD_MONGODB_PASSWORD}@cluster0.9zzrld0.mongodb.net/${dataBase}`,
      {
    ...
    console.log(
      chalk.bgGreenBright(
        `Connected to Mongodb Cload DataBase Server, DataBase: ${dataBase}`
      )
    );
  } catch (error) {
    console.log(chalk.red("Error:" + error));
